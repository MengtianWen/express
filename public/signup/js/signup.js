...
;(function (window) {
    ...
    AS.M = (function () {
        ...
        var available = function (str, callback) {
            var params = {
                url: '/signups/verify',
                data: {
                    query: str,
    ...
    AS.V = (function () {
        ...
        var show = {
            nextPage: function () {
                $('.signups-input').fadeOut(350);
                $('.signups-active').delay(350).slideDown();
            },
            prePage: function () {
                $('.signups-active').fadeOut(350);
                $('.signups-input').delay(350).slideDown();
            },
            success: function () {
                $('.signups-active').fadeOut(350);
                var signupres = $('.signups-res');
                var html =
                    '<div class="signups-success">' +
                    '   <div class="signups-success--bc"></div>' +
                    '   <div class="signups-success--txt">恭喜您注册成功！</div>' +
                    '   <div class="signups-success--txt">马上跳转...</div>' +
                    '</div>';
            ...
            failure: function () {
                ...
                var signupres = $('.signups-res');
                var html =
                    '<div class="signups-failure">' +
                    '   <div class="signups-failure--bc"></div>' +
                    '   <div class="signups-failure--txt">很抱歉注册失败！</div>' +
                    '   <div class="signups-failure--txt">请稍后再试...</div>' +
                    '</div>';
                signupres.append(html);
                signupres.delay(350).fadeIn();
            }
    ...
    AS.C = (function () {
        var M = AS.M;
        var V = AS.V;
        var inputArr = $('.signups-comm > input');
        var check = {
            id: false,
            email: false,
            password: false,
        ...
        var verify = {
            ...
            id: function () {
                ...
                if (/^201[3-9][0-9]{5,6}$/.test(id)){
                    M.available(id, function (ok, res) {
            ...
            email: function () {
                ...
                if (/^[a-z0-9]+([._\\-]*[a-z0-9])*@([a-z0-9]+[-a-z0-9]*[a-z0-9]+.){1,63}[a-z0-9]+$/.test(email)){
                    M.available(email, function (ok, res) {
